...
public class PreRetrieveGUI extends JFrame implements ActionListener
{
	...
	public PreRetrieveGUI(SteganoInformation info, boolean operation)
	{
		...
		if(features == CPT.UUM || features== CPT.UEM
				|| features == CPT.CUM || features== CPT.CEM)
		...
		if(features == CPT.CUM || features== CPT.CUF
				|| features == CPT.CEM || features== CPT.CEF)
		...
		if(features== CPT.UEM || features== CPT.CEM
				|| features== CPT.UEF || features== CPT.CEF)
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else
		{
			...
			if(operation== RETRIEVE_MESSAGE	&&
					lblContainsWhat.getText().equals("Embedded filez"))
			{
			...
			if(lblContainsWhat.getText().equals("Embedded message"))
			{
				...
				else
				{
					...
					if(message!= null && message.equals("Incorrent Password"))
						...
						JOptionPane.showMessageDialog(null, "Error!\n"+ CPT.getMessage(), "Oops, Error!", JOptionPane.ERROR_MESSAGE);
				}
			}
			else
			{
                            
				//boolean res= CPT.retrieveFile(info, password, false);
				if(true)
				{
					String message= CPT.getMessage();
					if(message!= null)
						if(message.equals("Incorrent Password"))
							JOptionPane.showMessageDialog(null, "Incorrect Password Specified!\nMake sure that Caps lock is not on.", "Invalid password!", JOptionPane.WARNING_MESSAGE);
							 ...
							 {							
								...
								//CPT.retrieveFile(info, password, true);
								showFile(info);
							 }
							 else
								JOptionPane.showMessageDialog(null, "Error!\n"+ CPT.getMessage(), "Oops, Error!", JOptionPane.ERROR_MESSAGE);
...
class MessageDisplay extends JFrame
{
	...
	{
		super("Retrieved message from file '"+ fileName+ "' - Steganograph "+ CPT.VERSION+ "    -- By: vinoth");
		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
