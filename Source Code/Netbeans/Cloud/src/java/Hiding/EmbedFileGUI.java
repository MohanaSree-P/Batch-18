...
public class EmbedFileGUI extends JFrame implements ActionListener, ItemListener
{
	...
	public EmbedFileGUI(BackEndHandler client)
	{
		...
		panelFeatures1b.add(new MyJLabel("", arialFont, Color.blue, Color.lightGray));
		panelFeatures1b.add(sliderCompression);
		panelFeatures1b.add(new MyJLabel("Choose Check Book If Encryption Needs", arialFont, Color.blue, Color.lightGray));
		...
		panelFeatures1= UtilityOperations.createBorderedPanel(panelFeatures1, "", 10, 3);
 ...
 public BufferedImage grayOut(BufferedImage img) {
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(source== btnHelp)
                {  JOptionPane.showMessageDialog(this, "Sorry", "Invalid password!", JOptionPane.WARNING_MESSAGE);
}
		...
		if(source== btnGo)
		{
			...
			if(CPT.embedFile(client.getMasterFile(), client.getOutputFile(), client.getDataFile(), compression, password))
				JOptionPane.showMessageDialog(this, CPT.getMessage(), "Operation Successful", JOptionPane.INFORMATION_MESSAGE);
			else
				JOptionPane.showMessageDialog(this, CPT.getMessage(), "Operation Unsuccessful", JOptionPane.ERROR_MESSAGE);
		}
	}
