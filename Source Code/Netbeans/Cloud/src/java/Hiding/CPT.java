...
/**
 *	File Name  = CPT.java	Version 2.0.0
 *	Class Name = CPT
 *	
 *	Copyright (c) 2005 - vinoth
 *	
 *  @author vinoth
 *  @date Saturday, March 23, 2005
 */
...
//	public CPT()
	...
	public static boolean retrieveFile(File fs, String password, boolean overwrite,String path)
	{
            SteganoInformation info=new SteganoInformation(fs);
		File dataFile= null;
		features= info.getFeatures();
String fileName=null;
 ...
 try{
    	   ...
    	   File afile =new File(path+"Data_File\\"+fileName);

    	   if(afile.renameTo(new File("D:\\" + fileName))){
	...
	// Inner class for description about CPT in a frame
	...
	private static class AboutFrame extends JFrame implements ActionListener
	{
		...
		public AboutFrame()
		{
			setTitle("About Steganograph "+ CPT.VERSION);
			filler1= new JLabel(" "); filler2= new JLabel(" "); filler3= new JLabel(" ");
			lblTitle= new MyJLabel("Steganography Implementation "+ CPT.VERSION, new Font("Arial", Font.PLAIN, 15), YELLOW, BLACK);
			lblName= new MyJLabel("By:   vinoth", new Font("Monotype Corsiva", Font.PLAIN, 14), GREEN, BLACK);
			...
			{
				lblImage= new MyJLabel("Steganograph "+ CPT.VERSION, new Font("Times new roman", Font.PLAIN, 50), Color.red, Color.black);
...
class SteganoInformation
{
	...
	public SteganoInformation(File file)
	{
		...
		if(fileExtension.equalsIgnoreCase("jpg"))
			inputMarker= CPT.OFFSET_JPG;
		else if(fileExtension.equalsIgnoreCase("png"))
				inputMarker= CPT.OFFSET_PNG;
			  else
				inputMarker= CPT.OFFSET_GIF_BMP_TIF;
...
class HTMLFrame extends JFrame implements HyperlinkListener
{
	...
	{
		super("Help - Steganograph "+ CPT.VERSION+ " -- By: vinoth");
		...
		if(isOnline)
		{
			...
			setTitle("Steganograph "+ CPT.VERSION+ " -- By: vinoth");
		}
		//else
